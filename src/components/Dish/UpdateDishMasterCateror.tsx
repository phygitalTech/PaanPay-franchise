...
// Type definition for form values based on schema
type FormValues = z.infer<typeof DishUpdateSchema>;

// Interface for formatted dish data used in cards (People, KG, Price)
interface CardData {
  people: string;
  kg: string;
  price: string;
...
const UpdateDishMasterCateror: React.FC = () => {
  ...
  const selectedDishId = watch('name');
  // Watching the 'name' field (selected dish)

  const {data: DishData} = useGetDishes();
  // console.log('DishData', DishData);

  const formattedDishDropdownData = DishData?.data.dishes.map(
  ...
  const {data: dishData, isLoading: isDishLoading} =
    useGetDishById(selectedDishId);
  console.log('dishData selected :', dishData);
  ...
  const {
    data: dishRawMaterialData = {data: {caterorDishRawMaterialQuantities: []}},
  } = useGetDishRawMaterials(selectedDishId, selectedPeople);

  console.log('dishRawMaterialData', dishRawMaterialData);

  const mappedData =
    dishRawMaterialData?.data?.caterorDishRawMaterialQuantities.map(
      (item: {
      ...
      <div className="overflow-x-auto">
        ...
        <FormProvider {...methods}>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-10">
            ...
            <div className="col-span-2 grid grid-cols-1 gap-4 overflow-x-auto md:col-span-6 md:grid-cols-6">
...
export default UpdateDishMasterCateror;
