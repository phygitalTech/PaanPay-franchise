...
import {
  useGetCompletedOrderDetails,
  useGetPendingOrderDetails,
} from '@/lib/react-query/Admin/dashboard';
...
const PendingOrdersReport = () => {
  ...
  const {data: pendingOrders} = useGetPendingOrderDetails(user?.id!);
  const {data: AllMerchant} = useGetMerchant(user?.id!);
  console.log('merchant', AllMerchant);
  console.log('completed', pendingOrders);
  ...
  const tableData =
    pendingOrders?.data?.pendingOrders?.flatMap((order) => {
      const orderDate = new Date(order.orderedAt);
      const extras =
